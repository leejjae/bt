...
parser.add_argument('--max_thresh_p', type=float, default=2.0, help='maximum of threshold for labeled (default 2.0)')
parser.add_argument('--max_thresh_n', type=float, default=2.0, help='maximum of threshold for unlabeled (default 2.0)')
...
def pre_train(classifier, train_dataset, val_dataset, test_dataset, args):
    ...
    #         optimizer, num_warmup_steps=n_warmup, num_training_steps=epochs
    #     )
    # else:
    #     scheduler = transformers.get_constant_schedule_with_warmup(
    #         optimizer, num_warmup_steps=n_warmup
    #     )
    ...
    for epoch in range(epochs):
        ...
        for data, true_labels, labels in train_loader:
            ...
            # 간단한 epoch 평균 loss만 기록
            bs = data.size(0)
            epoch_loss_sum += loss.item() * bs
            epoch_num += bs

        # scheduler.step()
        train_loss = epoch_loss_sum / max(1, epoch_num)
        history_loss.append(train_loss)

        # ======= Eval (val/test) with evaluate() =======
        # evaluate(model, dataset, args) 형태라 가정하고 loader.dataset 전달
        val_loss,  val_acc,  val_f1,  val_auc  = evaluate(classifier, val_loader.dataset,  args)
        test_loss, test_acc, test_f1, test_auc = evaluate(classifier, test_loader.dataset, args)
        ...
        print(
            ...
            f"val_loss={val_loss:.4f}  val_acc={val_acc:.4f}  val_macroF1={val_f1:.4f}  val_auc={val_auc:.4f}  "
            f"test_loss={test_loss:.4f}  test_acc={test_acc:.4f}  test_macroF1={test_f1:.4f}  test_auc={test_auc:.4f}"
        )

    # restore best (by val_acc)
    classifier.load_state_dict(best_val_classifier)

    # keep history (loss만 유지)
    history = {'loss': history_loss}

    # ======= Save classifier (original logic kept) =======
...
def train_episode(classifier, weighted_train_dataloader, test_loader, args):
    ...
    for inner_epoch in range(args.inner_epochs):
        for step, (data, true_labels, labels, weights) in enumerate(weighted_train_dataloader):
            ...
            logits = classifier(batch).view(-1)
            ...
            # print(f'logits_x.shape: {logits_x.shape}')
            ...
            target, mask = make_labels_from_fixmatch(logits_u_w, tau=0.95, T=1)
...
def train(classifier, positive_dataset, unlabeled_dataset, val_dataset, tot_test_dataset, test_dataset, args):
    ...
    for episode in range(epochs): 
        ...
        # cur_data : data
        # cur_labels : pos와 neg (모든 unl을 neg라고 간주)
        # cur_true_labels : pos와 unl
...
def prepare_and_run(args):
    ...
    pos_dataset, unl_dataset, train_dataset, val_dataset, test_dataset = data_manger.get_data(merge=False)
    ...
    if args.pretrained:
        ...
        pre_train(classifier, weak_train_x_dataset, weak_val_x_dataset, test_dataset, args)
